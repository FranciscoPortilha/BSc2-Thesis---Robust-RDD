...
for i in range(len(df_02)):
    weight = int(df_02.iloc[i][0][80:84])
    death = int(df_02.iloc[i][0][0])
    if (1415 <= weight) & (weight <= 1585):
        Weights = np.append(Weights, weight)
        if death == 2:
            Deaths = np.append(Deaths, 0)
...
print(jointFitRD("Donut", sample, cutoff=1500, b=85, outliers=False,donut=2).summary())
